...
```
{endpoint}viber/suppressions/unsubscribers
```
...
```json
{
    ...
    "links": {
        "first": "{endpoint}viber/suppressions/unsubscribers?page=1",
        "last": "{endpoint}viber/suppressions/unsubscribers?page=1",
        "prev": null,
        "next": null
    },
    "meta": {
        ...
        "links": [
            ...
            {
                "url": "{endpoint}viber/suppressions/unsubscribers?page=1",
                "label": "1",
                "active": true
            },
            {
        ...
        "path": "{endpoint}viber/suppressions/unsubscribers",
        "per_page": 15,
        "to": 6,
        "total": 6
    }
...
| type     | Mixed    | Type of the receivers (tag, category, account, all) |
...
```
{endpoint}viber/suppressions/unsubscribers/{id}
```
...
```
curl -X PUT \
  '{endpoint}viber/suppressions/unsubscribers/848c22a0-6969-455a-b5dc-4b119016b55b' \
  -H 'Accept: application/json' \
  -H 'Authorization: Bearer 7160f04c0587xxxxxxxxxxxxxxxx' \
  -H 'Content-Type: application/json' \
  -d '{
...
```
curl -X GET \
  '{endpoint}viber/suppressions/unsubscribers/0e3fdaad-2b46-4dcf-8b14-aafffbxxxxxx' \
    -H 'Accept: application/json' \
    -H 'Authorization: Bearer 7160f04c0587xxxxxxxxxxxxxxxx'
```
...
```
{endpoint}/viber/suppressions/unsubscribers/import
```
